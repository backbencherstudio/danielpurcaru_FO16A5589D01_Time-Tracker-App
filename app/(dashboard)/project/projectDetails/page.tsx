...
const Page = () => {
  ...
  const totalPages = Math.ceil(projectData.employees.length / itemsPerPage);
  ...
  const totalHours = projectData.employees.reduce((acc, emp) => acc + emp.Hours, 0);
  const totalCost = projectData.employees.reduce((acc, emp) => acc + parseFloat(emp.Cost.replace('$', '').replace(',', '')), 0);
      ...
      <div className="space-y-6   p-6 rounded-2xl ">
        <div className="flex flex-col  items-start w-full ">
          <div className='flex gap-2 items-center'>
            ...
            <h2 className="text-2xl font-semibold text-neutral-800">Project Name: {projectData.projectName}</h2>
          </div>
          <div className='flex w-full mt-4 gap-6'>
            <div className=' w-full'><DashboardCard title={'Total Hours'} value={totalHours} /></div>
            <div className=' w-full'><DashboardCard title={'Total Cost'} value={totalCost} /></div>
            <div className=' w-full'><DashboardCard title={'Project Status'} value={projectData.status.toLocaleString()} /></div>
          </div>
        </div>
